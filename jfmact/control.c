...
void CONTROL_ApplyAxis(int32 axis, ControlInfo *info, controldevice device)
{
    ...
    if (info->dyaw) printf("dyaw=%ld\n", info->dyaw);
    if (info->dx)     printf("dx=%ld\n", info->dx);
    if (info->dpitch)     printf("dpitch=%ld\n", info->dpitch);
    if (info->dy)     printf("dy=%ld\n", info->dy);
    if (info->droll)     printf("droll=%ld\n", info->droll);
    if (info->dz)     printf("dz=%ld\n", info->dz);
}

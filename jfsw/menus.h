...
typedef struct MENU_ITEM
    ...
    void *child;                        // Should be menugroup, used to spawn
                                        // sub-groups from inventoryItems.
    ...
    long tics;                          // Ticcount for item
     ...
     BOOL (*postprocess)(struct MENU_ITEM *); // Can do things on inventoryItems select
    } MenuItem, *MenuItem_p;
...
typedef struct
    ...
    char *text;
    MenuItem_p items;                   // Array of menu inventoryItems for this menu.

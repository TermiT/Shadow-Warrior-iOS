...
#define HEALTH_ERASE 2401
    ...
    iphone_notifyPlayerInfoChanged(PI_HEALTH);
...
#define AMMO_ERASE 2404
    ...
    iphone_notifyPlayerInfoChanged(PI_AMMO);
...
PlayerUpdateWeapon(PLAYERp pp, short WeaponNum)
    ...
    iphone_notifyPlayerInfoChanged(PI_WEAPON);
    if (gs.BorderNum < BORDER_BAR || pp - Player != screenpeek)
        return;
...
#define ARMOR_ERASE 2401
    ...
    iphone_notifyPlayerInfoChanged(PI_ARMOR);
    if (gs.BorderNum < BORDER_BAR || pp - Player != screenpeek)
        return;
...
#define KEYS_ERASE 2402
    ...
    iphone_notifyPlayerInfoChanged(PI_KEYS);
        
    if (gs.BorderNum < BORDER_BAR || pp - Player != screenpeek)
        return;

    xsize = tilesizx[PANEL_KEY_RED]+1;
        ...
        if (TEST(psp->flags, PANF_KILL_AFTER_SHOW) && !TEST(psp->flags, PANF_NOT_ALL_PAGES))
            ...
            if (is_iPhone5) {
                switch (picnum) {
                    ...
                    case SWORD_SWING1:
                    case SWORD_SWING2:
                    case BLOODYSWORD_REST:
                    case BLOODYSWORD_SWING0:
                    case BLOODYSWORD_SWING1:
                    case BLOODYSWORD_SWING2:
                        x1 += 60;
                        x2 += 60;
                        break;
                    default:

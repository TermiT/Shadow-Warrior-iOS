...
NewLevel(VOID)
{
    ...
    {
		if (FinishAnim == ANIM_ZILLA || (FinishAnim == ANIM_SERP && isFullGame == 0))
			MenuLevel();
    }
    FinishAnim = 0;
}
...
#define CREDITS2_PIC 5118
    ...
    ResetKeys();
    StopSong();
    iphone_notifyGameOver();
}

VOID
SharewareCredits(VOID)
{
    char called;
    long fin;
    int i;
...
#define CREDITS2_PIC 5118
    ...
    ResetKeys();
    StopSong();
    iphone_notifySharewareGameOver();
}
...
#if 0 // disable music when menu starts
	if (gs.MusicOn)
    {
...
#if 1
    iphone_notifyLevelLoading(level_name);
    return; // IOS
#else
    DrawLoadLevelScreen();
...
#define BONUS_REST_TICS 50
    ...
    long Tics = 0;
    long line = 0;
//    BOOL BonusDone; // now it's global
    
    if(Level < 0) Level = 0;

    intermission_info_t intermission_info = {
            .level_secrets = LevelSecrets,
            .found_secrets = Player->SecretsFound,
            .kills = Player->Kills,
            .total_killable = TotalKillable,
    };

    strcpy(intermission_info.map_name, LevelInfo[PlayingLevel].Description);
    second_tics = (PlayClock/120);
    minutes = (second_tics/60);
    seconds = (second_tics%60);
    sprintf(intermission_info.user_time, "%2ld : %02ld", minutes, seconds);
    strcpy(intermission_info.best_time, LevelInfo[PlayingLevel].BestTime);
    strcpy(intermission_info.par_time, LevelInfo[PlayingLevel].ParTime);

    iphone_notifyIntermissionStart(&intermission_info);
...
RunLevel(VOID)
{
    ...
    iphone_notifyLevelInitialized(Level);
...
#else
	if (SW_SHAREWARE)
		printf("Usage: sw [options]\n");
	else
        printf("Usage: sw [options] [map]\n");
...
#define SET_LOC_KEY(loc, sync_num, key_test) SET(loc, ((!!(key_test)) << (sync_num)))
    ...
    if (info.dpitch > 0)
        ...
        aimvel = labs((-info.dpitch) >> 10);
    else
        aimvel = info.dpitch >> 10;
    ...
    momx = mulscale(loc->vel, sintable[NORM_ANGLE(newpp->pang + 512)], 9);
    momy = mulscale(loc->vel, sintable[NORM_ANGLE(newpp->pang)], 9);
    
    momx += mulscale(loc->svel, sintable[NORM_ANGLE(newpp->pang)], 9);
    momy += mulscale(loc->svel, sintable[NORM_ANGLE(newpp->pang + 1536)], 9);

...
DoShadows(SPRITEp tsp, long viewz)
    ...
    // Check for voxel inventoryItems and use a round generic pic if so
    //DoVoxelShadow(new);
...
//    ScreenBPP    = bpp;
    ...
    return((long)setgamemode(ScreenMode,ScreenWidth,ScreenHeight,ScreenBPP));
      }

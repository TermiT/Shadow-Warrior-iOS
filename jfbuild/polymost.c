...
void polymost_texinvalidate (long dapicnum, long dapalnum, long dameth, int flags)
{
	PTIter iter;
	PTHead * pth;
	...
	while ((pth = PTIterNext(iter)) != 0) {
		if (pth->pic[PTHPIC_BASE]) {
			pth->pic[PTHPIC_BASE]->flags |= PTH_DIRTY|flags;
		}
			...
			for(;ix0<=ix1;ix0++)
			{
				...
				do {
					...
					for(i=0;i<nn;i++)
					{
						...
						bglVertex3f((ox-ghalfx)*r*grhalfxdown10x,(ghoriz-oy)*r*grhalfxdown10,r*(1.0/1024.0));
					}
					bglEnd();
		...
		else
		{
			...
			do {
				...
				for(i=0;i<n;i++)
				{
					...
					bglVertex3f((px[i]-ghalfx)*r*grhalfxdown10x,(ghoriz-py[i])*r*grhalfxdown10,r*(1.0/1024.0));
				}
				bglEnd();
...
void polymost_drawsprite (long snum)
{
	...
	switch((globalorientation>>4)&3)
	{
		...
		case 1: //Wall sprite
			...
			px[3] = sx0; py[3] = sf0;
			pow2xsplit = 0;
            drawpoly(px,py,4,method | METH_LAYERS);
			break;
		case 2: //Floor sprite

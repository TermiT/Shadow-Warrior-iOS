...
static void ptm_rgba_to_rgb(const GLvoid *rgba, GLvoid *rgb, unsigned int len) {
    for (unsigned int i = 0; i < len; i++) {
        unsigned char *src = ((unsigned char*)rgba) + i*4;
        unsigned char *dst = ((unsigned char*)rgb) + i*3;
        dst[0] = src[0];
        dst[1] = src[1];
        dst[2] = src[2];
    }
...
static void ptm_rgba_to_rgb565(const GLvoid *rgba, GLvoid *rgb565, unsigned int len) {
    unsigned int *inPixel32 = (unsigned int*)rgba;
    unsigned short *outPixel16 = (unsigned short*)rgb565;
    for(int i = 0; i < len; ++i, ++inPixel32) {
...
static void ptm_rgba_to_rgba4444(const GLvoid *rgba, GLvoid *rgba4444, unsigned int len) {
    unsigned int *inPixel32 = (unsigned int*)rgba;
    unsigned short *outPixel16 = (unsigned short*)rgba4444;
    for(int i = 0; i < len; ++i, ++inPixel32) {
        const unsigned int r = ((*inPixel32 >> 0 ) & 0xFF);
        const unsigned int g = ((*inPixel32 >> 8 ) & 0xFF);
        const unsigned int b = ((*inPixel32 >> 16) & 0xFF);
        const unsigned int a = ((*inPixel32 >> 24) & 0xFF);
        *outPixel16++ = ((r >> 4) << 12) | ((g >> 4) << 8) | ((b >> 4) << 4) | ((a >> 4) << 0);
    }
}
...
static void ptm_uploadtexture(PTMHead * ptm, unsigned short flags, PTTexture * tex, PTCacheTile * tdef)
{
    ...
    intexfmt = tex->hasalpha ? GL_RGBA : GL_RGB;

    if (ptm->flags & PTH_3DVIEW) {
        dont_convert = 1;
    }
	...
	for ( ; mipmap > 0 && (tex->sizx > 1 || tex->sizy > 1); mipmap--) {
		ptm_mipscale(tex);
		ptm_fixtransparency(tex, (flags & PTH_CLAMPED));
	}
    
    comprdata = malloc(tex->sizx * tex->sizy * 3);
    
	for (mipmap = 0; tex->sizx > 1 || tex->sizy > 1 || mipmap == 0; mipmap++) {
        if (mipmap != 0) {
            ptm_mipscale(tex);
            ptm_fixtransparency(tex, (flags & PTH_CLAMPED));
        }

        if (intexfmt == GL_RGB && !dont_convert) {
            if (is_hiEnd) {
                ptm_rgba_to_rgb(tex->pic, comprdata, tex->sizx*tex->sizy);
                bglTexImage2D(GL_TEXTURE_2D, mipmap,
                              GL_RGB, tex->sizx, tex->sizy, 0, GL_RGB,
                              GL_UNSIGNED_BYTE, comprdata);
                numimages++;

            } else {
                ptm_rgba_to_rgb565(tex->pic, comprdata, tex->sizx*tex->sizy);
                bglTexImage2D(GL_TEXTURE_2D, mipmap,
                              GL_RGB, tex->sizx, tex->sizy, 0, GL_RGB,
                              GL_UNSIGNED_SHORT_5_6_5, comprdata);
                numimages++;
            }
        } else {
            if (is_hiEnd || dont_convert) {
                bglTexImage2D(GL_TEXTURE_2D, mipmap,
                              tex->rawfmt, tex->sizx, tex->sizy, 0, tex->rawfmt,
                              GL_UNSIGNED_BYTE, tex->pic);
                numimages++;
            } else {
                ptm_rgba_to_rgba4444(tex->pic, comprdata, tex->sizx*tex->sizy);
                bglTexImage2D(GL_TEXTURE_2D, mipmap,
                              tex->rawfmt, tex->sizx, tex->sizy, 0, tex->rawfmt,
                              GL_UNSIGNED_SHORT_4_4_4_4, comprdata);
                numimages++;
            }
        }
    };
...
void ptm_set_filter_mode(GLuint glpic) {
//    gltexfiltermode = mode;
//    for (int i = 0; i < PTMHASHHEADSIZ; i++) {
//        PTHash *pth = pthashhead[i];
//        if (pth != NULL) {
//            for (int j = 0; j < PTHPIC_SIZE; j++) {
//                PTMHead *head = pth->head.pic[j];
//                if (head != 0 && head->glpic != 0) {
//                    glBindTexture(GL_TEXTURE_2D, head->glpic);
//                    bglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, glfiltermodes[mode].mag);
//                    bglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, glfiltermodes[mode].min);
//                }
//            }
//        }
//    }

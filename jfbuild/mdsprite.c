...
#if defined(__POWERPC__) || TARGET_CPU_ARM
...
unsigned gloadtex (long *picbuf, long xsiz, long ysiz, long is8bit, long dapal)
{
	...
	bglTexImage2D(GL_TEXTURE_2D,0, GL_RGBA, xsiz,ysiz,0,GL_RGBA,GL_UNSIGNED_BYTE,(unsigned char *)pic2);
	free(pic2);
	return(rtexid);
}
...
static void setzrange0 (long *lptr, long z0, long z1)
{
	long z, ze;
	if (!((z0^z1)&~31)) {
        lptr[z0>>5] &= ((~(-1<<SHIFTMOD32(z0)))|(-1<<SHIFTMOD32(z1)));
        return;
    }
	z = (z0>>5);
    ze = (z1>>5);
...
static void setzrange1 (long *lptr, long z0, long z1)
{
	long z, ze;
	if (!((z0^z1)&~31)) {
//        long v1 = SHIFTMOD32(z1);
//        long v2 = -1 << v1;
//        long v3 = SHIFTMOD32(z0);
//        long v4 = -1 << v3;
//        long v5 = ~v2;
//        long v6 = v5 & v4;
//        long i = z0>>5;
...
#endif
            int fxx = (xx > vptr[j].x * 2) ? 1 : 0;
            int fyy = (yy > vptr[j].y * 2) ? 1 : 0;
            int fzz = (zz > vptr[j].z * 2) ? 1 : 0;
            fp.x = ((float) vptr[j].x) - phack[fxx] + phack[1 - fxx];
            fp.y = ((float) vptr[j].y) - phack[fyy] + phack[1 - fyy];
            fp.z = ((float) vptr[j].z) - phack[fxx] + phack[1 - fzz];

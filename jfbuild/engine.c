...
void setviewback(void)
{
	...
	if (setviewcnt == 0) {
		...
		invalidatetile(baktile,-1,-1, PTH_3DVIEW);
	}
#endif
...
//
void invalidatetile(short tilenume, long pal, long how, int flags)
{
#if defined(POLYMOST) && defined(USE_OPENGL)
	...
	for (hp = 0; hp < 8; hp+=4) {
		...
		for (np = firstpal; np < firstpal+numpal; np++) {
			polymost_texinvalidate(tilenume, np, hp, flags);
		}
	}
#endif
}
